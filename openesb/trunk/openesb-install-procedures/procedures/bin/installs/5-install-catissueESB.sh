...
#
# Pre-reqs:
#   0-prerequisites.sh has been run for minor OS artifacts
#   1-acquisitions.sh has been run at some point to acquire the install artifacts.
#   3-install-jdk.sh has been run to install the Java jdk.
#   4-install-glassfish.sh has been run to install the Glassfish server.
#
...
#=========================================================================== 
echo "About to install caCatissueESB within the Glassfish web server..."
echo ""
...
#
# Copy the zip file into Glassfish deploy directory...
print_message "" $LOG_FILE
print_message "Deploying into Glassfish..." $LOG_FILE
GLASSFISH_DEPLOY_DIRECTORY=glassfish/domains/domain1/jbi/autoinstall
cp $WORK_DIR/$ACQUISITIONS_DIRECTORY/$DEPLOYED_ARTIFACT \
   $OPENESB_INSTALL_DIRECTORY/glassfish/$GLASSFISH_DEPLOY_DIRECTORY
exit_if_bad $? "Could not copy caCatissueESB.zip into $OPENESB_INSTALL_DIRECTORY/glassfish/$GLASSFISH_DEPLOY_DIRECTORY" $LOG_FILE

#=========================================================================
# If we got this far, we must be successful...
#=========================================================================
print_message "caCatissueESB installation completed." $LOG_FILE
